...
Second,  in  contrast to a previous re-analysis of an experimental pragmatics eye-tracking dataset \cite{QingLD2018}, the current re-analysis of \citeA{sun2020}'s data offers clear support for the Referential Belief link.  We believe this difference is most likely the result of participants's greater expectations for the observed instructions in SB2020's study, though this requires further investigation. %Further support for this explanation comes from \citeA{kreiss2020}, who found that the size of contrastive inferences is robustly predicted by empirically elicited production expectations.  (see preliminary production results from Qing et al, but also Kreiss and Degen 2020 results, plus tons of other literature supporting tight link between production and comprehension in general, and importance of clearly nameable items (ie, clear production expectations) for VW eye-tracking in particular (go back to mike's papers from the oughts) 
...
% COMMENTS TO ADDRESS
%the authors suggest that web-based eye-tracking may be useful for psycholinguistic research, but only if precise timing is not required – but 700ms is a big difference in psycholinguistics, even for "non-precise" timing. I think it would be more helpful for the discussion to focus on the timing issues and what may lead to the 700ms difference, rather than on general noise-reduction issues (the timing difference may be partially explained by noise, but I don't think entirely – and in either case, I think a shift in focus to the timing issue would be useful).
