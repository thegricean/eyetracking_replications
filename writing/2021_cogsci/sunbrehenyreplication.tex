...
\begin{abstract}
Experimental investigation is fundamental to theory-building in cognitive science, but its value depends on the \emph{linking assumption}: the assumption made by researchers about the mapping between empirical measurements and theoretical explanations. %Empirical data only inform theories if the linking assumption is sufficiently clear and justified. 
We argue that sufficient clarity and justification are often lacking for linking assumptions made in \emph{visual world eye-tracking}, a widely used experimental method in psycholinguistic research. We highlight the successes of this method within experimental pragmatics, which suffers particularly acutely from a lack of clear and justified linking assumptions. We then test what we term the \emph{Referential Belief} linking assumption: that the proportion of looks to a referent in a time window reflects participants' degree of belief that the referent is the intended target in that time window. We do so by eye-tracking data against explicit beliefs: in \expref{1}, we test an existing dataset on scalar implicature processing \cite<\expref{3} of>{sun2020} against explicit beliefs we collected in an incremental decision task. In \expref{2}, we replicate \citeA{sun2020} in a web-based eye-tracking paradigm using \texttt{WebGazer.js}. 
...
\dl{commenting  intro but leaving here in case parts are needed}
