...
\textbf{Data analysis}. \citeA{sun2020} fit separate linear regression models to target advantage scores in time windows of interest. We instead fit logistic mixed effects models predicting target selections. This choice was motivated by logistic regression being the more principled approach to modeling categorical data. It also avoids problems like pre-aggregating data and adding smoothing terms to avoid division by zero or discarding mathematically problematic data points. %\jd{The results are qualitatively identical if we instead run linear models on target preference scores. (??)}
...
\begin{table}
\caption{Overview of critical effects in determiner (det.) and name windows in \citeA{sun2020}, our \expref{1} and our \expref{2}. Rows correspond to model predictors. ``+": significantly positive effect; ``--": significantly negative effect; ``$\cdot$": no evidence of an effect.}
{\small
...
\textbf{Comparison with \citeA{sun2020}: replication analysis}. \jd{fill in}
