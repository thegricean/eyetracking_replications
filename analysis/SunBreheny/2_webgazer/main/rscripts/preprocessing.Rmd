...
nrow(participants) # 104 unique participants
...
mean(participants$subject_information.accuracy) # 69.73 (9.95)
sd(participants$subject_information.accuracy)

# distribution of camera views -- some people continued to see camera view even during experiment (bug we haven't figured out yet how to fix) 44 people :(
ggplot(participants, aes(x=subject_information.camblock,fill=subject_information.camblock)) +
  geom_histogram(stat="count")
prop.table(table(participants$subject_information.camblock)) # 42% continue to see camera :(
...
imgheight = 200  # boy/girl (taller than objects) + padding
...
dd = tmp %>%
  mutate(x_center = (system.windowW/2), 
         y_center = (system.windowH/2),
           ...
           AOI_topleft_x_min = x_center - (scene_width/2),# - wpadding,
           AOI_topleft_x_max = x_center - (scene_width/2) + imgwidth + wpadding,
           AOI_topleft_y_min = y_center + (scene_height/2) - imgheight - hpadding,
           AOI_topleft_y_max = y_center + (scene_height/2),# + hpadding,
         
           AOI_bottomleft_x_min = x_center - (scene_width/2),# - wpadding,
           AOI_bottomleft_x_max = x_center - (scene_width/2) + imgwidth + wpadding,
           AOI_bottomleft_y_min = y_center - (scene_height/2),# - hpadding,
           AOI_bottomleft_y_max = y_center - (scene_height/2) + imgheight + hpadding,
         
           AOI_topright_x_min = x_center + (scene_width/2) - imgwidth - wpadding,
           AOI_topright_x_max = x_center + (scene_width/2),# + wpadding,
           AOI_topright_y_min = y_center + (scene_height/2) - imgheight - hpadding,
           AOI_topright_y_max = y_center + (scene_height/2),# + hpadding,
         
           AOI_bottomright_x_min = x_center + (scene_width/2) - imgwidth - wpadding,
           AOI_bottomright_x_max = x_center + (scene_width/2),# + wpadding,
...
nrow(dd) # 191538 (with exclusions)
...
dd = tmp %>% 
  filter(look!="other") %>% 
  select(workerid,condition,size,list,slide_number,Name,Noun,click_onset_ms,gender_onset_ms,determiner_onset_ms,name_onset_ms,noun_onset_ms,look,target_look,competitor_look,center_look,webgazer_time,unixtlist,camview)
