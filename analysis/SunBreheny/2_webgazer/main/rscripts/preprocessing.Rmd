...
dd_determiner = dd_subset %>%
  ...
  mutate(target_look=as.factor(target_look),size=as.factor(size),time_fct=as.factor(time_fct)) %>%
  mutate(csize=as.numeric(size)-mean(as.numeric(size))) %>%
  mutate(ctime_fct=as.numeric(time_fct)-mean(as.numeric(time_fct)))
...
m.determiner = glmer(target_look ~ determiner*csize*ctime_fct + (determiner*csize*ctime_fct|workerid) + (1+determiner*csize*ctime_fct|Noun), family="binomial",data=dd_determiner)
summary(m.determiner)

m.name = glmer(target_look ~ determiner*csize*ctime_fct + (determiner*csize*ctime_fct|workerid) + (1+determiner*csize*ctime_fct|Noun), family="binomial",data=dd_name)
summary(m.name)
