...
dd_subset = dd_binned %>%
  select(workerid,condition,size,determiner,Name,Noun,webgazer_time,first_sample,relative_time,click_onset_ms_200shift,gender_onset_ms_200shift,determiner_onset_ms_200shift,name_onset_ms_200shift,noun_onset_ms_200shift,target_look,competitor_look) %>%
  mutate(determiner_window = ifelse(1000+determiner_onset_ms_200shift<relative_time,ifelse(relative_time<1000+name_onset_ms_200shift,1,0),0)) %>%
  mutate(name_window = ifelse(1000+name_onset_ms_200shift<relative_time,ifelse(relative_time<1000+noun_onset_ms_200shift,1,0),0)) %>%
  filter(target_look == 1 | competitor_look == 1)

dd_determiner = dd_subset %>%
  filter(determiner_window==1) %>%
  mutate(target_look=as.factor(target_look),size=as.factor(size),relative_time=as.factor(relative_time)) %>%
  mutate(csize=as.numeric(size)-mean(as.numeric(size))) %>%
  mutate(crelative_time=as.numeric(relative_time)-mean(as.numeric(relative_time)))

dd_name = dd_subset %>%
  filter(name_window==1) %>%
  mutate(target_look=as.factor(target_look),size=as.factor(size),relative_time=as.factor(relative_time)) %>%
  mutate(csize=as.numeric(size)-mean(as.numeric(size))) %>%
  mutate(crelative_time=as.numeric(relative_time)-mean(as.numeric(relative_time)))

m.determiner = glmer(target_look ~ determiner*csize*crelative_time + (determiner*csize*crelative_time|workerid) + (1+determiner*csize*crelative_time|Noun), family="binomial",data=dd_determiner)
summary(m.determiner)

m.name = glmer(target_look ~ determiner*csize*crelative_time + (determiner*csize*crelative_time|workerid) + (1+determiner*csize*crelative_time|Noun), family="binomial",data=dd_name)
summary(m.name)
